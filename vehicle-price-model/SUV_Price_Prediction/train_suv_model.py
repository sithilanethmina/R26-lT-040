"""
Multi-Brand SUV Price Prediction — Training Script
===================================================
Workspace : Model_training/
Script    : Model_training/SUV_Price_Prediction/train_suv_model.py
Dataset   : Model_training/Cleaned_Suv_Brands/

Key differences from Car model (train_combined_model.py):
  1. engine_cc is a REAL feature here (94% coverage, 0.334 correlation)
     Cars had engine_cc extracted from titles → unreliable → excluded
     SUVs have actual engine_cc data → critical price signal

  2. Fuel type is a REAL feature here
     Cars: 97% Petrol → fuel type useless
     SUVs: Petrol/Diesel/Hybrid/Electric → major price differentiator
       Electric  mean = 19.8M
       Hybrid    mean = 15.1M
       Diesel    mean = 13.9M
       Petrol    mean = 13.5M

  3. Price ceiling is 200M LKR (vs 100M for cars)
     Range Rover, Land Cruiser V8, Audi Q8 go above 100M

  4. 2025/2026 records KEPT — median prices match 2023/2024
     These are unregistered/new stock, not data errors

  5. Monotone constraints updated:
     engine_cc: +1 (bigger engine = more expensive)

  6. Minimum combo = 5 (vs 3 for cars)
     SUV market more diverse → need more data per combo
...
DATA_DIR   = BASE_DIR.parent / "Cleaned_Suv_Brands"
MODEL_DIR  = BASE_DIR / "models" / "suv"
OUTPUT_DIR = BASE_DIR / "outputs" / "suv"

RANDOM_STATE = 42

# ─── Brand Config ─────────────────────────────────────────────────────────────
# Brands with 80+ records → included in training
# Below 80 → too few for reliable per-brand evaluation
TRAINABLE_BRANDS = [
    "Toyota", "Honda", "Mitsubishi", "Nissan", "Kia",
    "Micro", "DFSK", "Suzuki", "Land-Rover", "Audi",
    "BMW", "Hyundai", "MG", "Daihatsu", "Peugeot",
    "Mahindra", "Range-Rover",
]

# Excluded brands (too few records for reliable training):
# Zotye=52, BYD=51, Jeep=46, Mercedes-Benz=44, Baic=37, Tata=37, Isuzu=28

# ─── Data Bounds ──────────────────────────────────────────────────────────────
PRICE_MIN         = 1_000_000    # 1M minimum — no cheap hatchbacks
PRICE_MAX         = 200_000_000  # 200M — covers Range Rover, Land Cruiser V8
YEAR_MIN          = 1990
YEAR_MAX          = 2026         # 2025/2026 kept — unregistered stock is valid
MILEAGE_MAX       = 500_000
ENGINE_CC_MIN     = 650          # smallest SUV engine (Suzuki Jimny)
ENGINE_CC_MAX     = 6_000        # largest (Land Cruiser V8)
MIN_COMBO_RECORDS = 2            # min records per brand+model+year

# ─── Features ─────────────────────────────────────────────────────────────────
# SUV-specific: engine_cc and fuel_type are real features (unlike car model)
...
# brand=0, model=0, variant=0, fuel_type=0, transmission=0,
# model_year=+1, mileage_km=-1, vehicle_age=-1, engine_cc=+1
MONOTONE_CONSTRAINTS = (0, 0, 0, 0, 0, 1, -1, -1, 0)


# ══════════════════════════════════════════════════════════════
#  NLP SCORING LAYER — Sri Lanka SUV Market Keywords
# ══════════════════════════════════════════════════════════════
...
def get_confidence(record_count: int) -> str:
    ...
    else:                    return "Low — insufficient SUV market data"
...
def generate_yearwise_summary(df: pd.DataFrame, output_path: Path):
    ...
    print(f"  Saved year-wise SUV market summary → {output_path.name}")
    return summary
...
def map_suv_generations(row):
    ...
    if model == "RAV4":
        if year <= 1999:
            ...
            row["variant"] = "2nd Gen"
        elif variant in ["Standard", "", "None"] and 2006 <= year <= 2012:
            row["variant"] = "3rd Gen"
        elif variant in ["Standard", "", "None"] and 2013 <= year <= 2018:
            row["variant"] = "4th Gen"
        elif variant in ["Standard", "", "None"] and year >= 2019:
            row["variant"] = "5th Gen"
        
    elif brand == "mitsubishi" and model in ["montero", "pajero"]:
        if year <= 1999: return "2nd Gen"
        elif 2000 <= year <= 2006: return "3rd Gen"
        else: return "4th Gen"
        
    elif brand == "honda" and model in ["cr-v", "crv"]:
        if year <= 2001: return "1st Gen"
        elif 2002 <= year <= 2006: return "2nd Gen"
        elif 2007 <= year <= 2011: return "3rd Gen"
        else: return "4th Gen"
        
    return variant

def load_and_clean_data() -> pd.DataFrame:
    print(f"Loading SUV datasets from: {DATA_DIR} ...")
    ...
    # ── Step 2: Price filter (1M – 200M LKR) ──
    before = len(combined)
    combined = combined[
    ...
    # ── Step 2.1: Remove fake lease downpayments and scam listings for Toyota/Honda SUVs ──
    suspicious_low_price = (combined["brand"].isin(["Toyota", "Honda"])) & (combined["price"] < 3500000)
    combined = combined[~suspicious_low_price].copy()

    # ── Step 3: Year filter (1990 – 2026) ──
    # 2025/2026 KEPT — unregistered/new stock, median prices match 2023/2024
    before = len(combined)
    combined = combined[
    ...
    # "1977" is a data entry error — replace with "Automatic"
    combined["transmission"] = combined["transmission"].astype(str).str.strip()
    combined.loc[
        ~combined["transmission"].isin(["Automatic", "Manual"]),
        "transmission"
    ] = "Automatic"

    # ── Step 6: Engine CC cleaning ──
    # SUV engine_cc is real data — clean it properly
    # Valid range: ENGINE_CC_MIN to ENGINE_CC_MAX
    invalid_cc = (
    ...
    # ── Step 8: Mileage imputation ──
    # Use brand+model+year median for SUVs (more specific than year-only)
    ...
    combined["variant"] = combined.apply(map_suv_generations, axis=1)
    ...
    # ── Step 13: Record count for confidence ──
...
def print_data_summary(df: pd.DataFrame):
    ...
    print(f"\n  Fuel type price impact (why fuel_type is a feature):")
    ft = df.groupby("fuel_type")["price"].agg(["count", "mean", "median"])
    for ft_name, r in ft.sort_values("mean", ascending=False).iterrows():
        print(f"    {ft_name:<12} N={int(r['count']):>5}  "
              f"mean={r['mean']/1e6:>5.1f}M  median={r['median']/1e6:>5.1f}M")

    print(f"\n  Engine CC price brackets (why engine_cc is a feature):")
    cc_df = df.copy()
    cc_df["cc_bracket"] = pd.cut(
        cc_df["engine_cc"],
        bins=[0, 1000, 1500, 2000, 2500, 3000, 4000, 6001],
        labels=["<1000", "1000-1500", "1500-2000",
                "2000-2500", "2500-3000", "3000-4000", ">4000"]
    )
    cc_grp = cc_df.groupby("cc_bracket")["price"].agg(["count", "mean"])
    for bracket, r in cc_grp.iterrows():
        print(f"    {str(bracket):<12} N={int(r['count']):>5}  "
              f"mean={r['mean']/1e6:>5.1f}M")
...
def train_all_models(df: pd.DataFrame):
    ...
    models = {
        ...
        # engine_cc: +1 (bigger engine = pricier) ← SUV specific
        # Random Forest may score lower MAE but lacks monotone constraints
        # 20K-50K LKR MAE difference is acceptable for logical consistency
    ...
    print(f"  Reason              : Monotone constraints ensure logical "
          f"SUV pricing (year↑ price↑, mileage↑ price↓, engine_cc↑ price↑)")
...
def print_sample_predictions(display_df):
    print(f"\n{'─'*95}")
    print("SAMPLE PREDICTIONS  (10 random SUV listings)")
...
def demonstrate_nlp(df, prod_model, encoder):
    print(f"\n{'='*70}")
    print("NLP SCORING LAYER — SUV Demonstration")
    print(f"{'='*70}")
...
def save_artifacts(results_df, trained, encoder, prod_model,
    ...
    joblib.dump(prod_model, MODEL_DIR / "best_suv_model.pkl")
    joblib.dump(encoder,    MODEL_DIR / "suv_ordinal_encoder.pkl")

    # CSVs
    results_df.to_csv(OUTPUT_DIR / "suv_model_comparison.csv",        index=False)
    display_df.to_csv(OUTPUT_DIR / "suv_test_predictions.csv",         index=False)
    cv_df.to_csv(     OUTPUT_DIR / "suv_cross_validation_results.csv", index=False)
    ...
    with open(OUTPUT_DIR / "suv_nlp_config.json", "w") as f:
    ...
    lookup.to_csv(OUTPUT_DIR / "suv_brand_model_lookup.csv", index=False)

    # Year-wise market summary
    generate_yearwise_summary(
        df_clean, OUTPUT_DIR / "suv_yearwise_summary.csv"
    )

    print(f"\n  models/suv/best_suv_model.pkl")
    print(f"  models/suv/suv_ordinal_encoder.pkl")
    print(f"  models/suv/[catboost/xgbregressor/gradient_boosting/random_forest].pkl")
    print(f"  outputs/suv/suv_model_comparison.csv")
    print(f"  outputs/suv/suv_test_predictions.csv")
    print(f"  outputs/suv/suv_cross_validation_results.csv")
    print(f"  outputs/suv/suv_nlp_config.json")
    print(f"  outputs/suv/suv_brand_model_lookup.csv")
    print(f"  outputs/suv/suv_yearwise_summary.csv")
