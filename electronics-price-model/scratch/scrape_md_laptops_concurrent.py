...
product_links = []
print("Fetching category pages from MD Computers (Laptops)...")

for page in range(1, 15):  # check up to 15 pages
    if page == 1:
        url = base_url
    else:
    ...
    try:
        ...
        if res.status_code != 200:
            print(f"Ended category pagination at page {page} (Status: {res.status_code})")
            break
...
print(f"Total laptop product links found: {len(product_links)}")
...
print("Scraping laptop details concurrently...")
with ThreadPoolExecutor(max_workers=8) as executor:
    ...
    for idx, future in enumerate(as_completed(futures)):
        ...
        if (idx + 1) % 10 == 0 or idx == len(product_links) - 1:
            print(f"Progress: [{idx+1}/{len(product_links)}] laptops fetched.")

# Save to CSV
csv_file = "scratch/scraped_md_laptops.csv"
keys = ['Title', 'Price', 'Location_Category', 'Link', 'Brand', 'Condition', 'RAM', 'Storage', 'Description']
with open(csv_file, 'w', newline='', encoding='utf-8') as f:
    writer = csv.DictWriter(f, fieldnames=keys)
    writer.writeheader()
    writer.writerows(scraped_data)

print(f"Successfully saved {len(scraped_data)} MD Computers laptops to {csv_file}")
