document.addEventListener("DOMContentLoaded", () => {
  ...
  const selectGpuManuallyBtn = document.getElementById("selectGpuManuallyBtn");
  ...
  async function initGpuMetadata() {
    ...
    try {
      const liveRes = await fetch(`${GATEWAY_URL}/api/gpu/metadata`, {
  ...
  function updateGpuVramOptions(modelEntry, preferredVram) {
    ...
    const vrams =
      modelEntry && modelEntry.valid_vrams && modelEntry.valid_vrams.length > 0
  ...
  function setupGpuAutocomplete() {
    ...
    function validateModelInput() {
      ...
      const match = (GPU_METADATA.models || []).find((m) => {
        ...
        return (
          m.model.toUpperCase() === val.toUpperCase() ||
          (mNorm && mNorm === normVal)
        );
      });
    ...
    modelInput.addEventListener("input", (e) => {
      ...
      const selectedManuf = manufacturerSelect
      ...
      const matches = (GPU_METADATA.models || []).filter((m) => {
        ...
        const mNorm = m.normalized || normalizeGpuModel(m.model);
        return (
          mUpper.includes(query) ||
          (mNorm && mNorm.includes(normQuery)) ||
          (normQuery && normQuery.includes(mNorm))
        );
      });
    ...
    modelInput.addEventListener("keydown", (e) => {
      ...
      if (items.length === 0 || suggestionsBox.classList.contains("hidden"))
        return;
  ...
  async function initMobileMetadata() {
    ...
    try {
      const liveRes = await fetch(`${GATEWAY_URL}/api/mobile/metadata`, {
        signal: AbortSignal.timeout(2000),
      }).catch(() => null);
      if (liveRes && liveRes.ok) {
        const liveData = await liveRes.json();
  ...
  document
    ...
    .addEventListener("change", (e) => initVehicleMetadata(e.target.value));
  document
    .getElementById("vehicleBrandSelect")
    .addEventListener("change", () => populateVehicleModels());
  ...
  selectGpuManuallyBtn.addEventListener("click", () => {
    categorySelect.value = "gpu";
    categorySelect.dispatchEvent(new Event("change"));
    const specEditor = document.getElementById("specEditor");
    const modelInput = document.getElementById("gpuModelInput");
    if (specEditor) specEditor.open = true;
    if (modelInput) {
      modelInput.value = "";
      modelInput.classList.remove("input-invalid");
      modelInput.focus();
    }
    selectGpuManuallyBtn.classList.add("hidden");
    showStatus(
      "Select the exact GPU model from the dataset before checking the price.",
    );
  });
      ...
      chrome.tabs.sendMessage(
        ...
        async (response) => {
          setLoading(
            extractBtn,
            false,
            '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4M7 10l5 5 5-5M12 15V3"/></svg> Extract',
          );
          ...
          if (response && response.success) {
            if (
              response.data.category === "unsupported" ||
              response.data.is_unsupported_item
            ) {
              showError(
  ...
  predictBtn.addEventListener("click", async () => {
    ...
    try {
      ...
      if (category === "vehicle") {
        ...
        if (vType === "suvs")
          endpoint = `${GATEWAY_URL}/api/vehicle/predict/suv`;
        if (vType === "vans")
          endpoint = `${GATEWAY_URL}/api/vehicle/predict/van`;
      }
      ...
      if (!response.ok) {
        ...
        throw new Error(
          errData.detail || "Gateway error. Ensure services are running.",
        );
      }
  ...
  function buildPayload(category) {
    if (category === "gpu") {
      ...
      const matched = (GPU_METADATA.models || []).find((m) => {
        ...
        const mNorm = m.normalized || normalizeGpuModel(m.model);
        return (
          m.model.toUpperCase() === modelVal.toUpperCase() ||
          (mNorm && mNorm === normVal)
        );
      });
      ...
      if (!matched) {
        showError(
          `Unknown GPU model: "${modelVal}". Please select a recognized model.`,
        );
        if (modelInput) modelInput.classList.add("input-invalid");
        return null;
      }
      ...
      return {
        ...
        stock: "In Stock",
        listed_price:
          !isNaN(listedPrice) && listedPrice > 0 ? listedPrice : null,
      };
      ...
      return {
        ...
        model: model,
        storage_gb:
          parseFloat(document.getElementById("mobileStorageInput").value) ||
          128,
        ram_gb:
          parseFloat(document.getElementById("mobileRamInput").value) || 6,
        warranty_days:
          parseFloat(document.getElementById("mobileWarrantyInput").value) || 0,
      };
    } else if (category === "vehicle") {
      const variant = document
        .getElementById("vehicleVariantInput")
        .value.trim();
      ...
      return {
        ...
        model: model,
        model_year:
          parseInt(document.getElementById("vehicleYearInput").value) || 2015,
        mileage_km:
          parseInt(document.getElementById("vehicleMileageInput").value) || 0,
        engine_cc:
          parseInt(document.getElementById("vehicleEngineCCInput").value) ||
          1000,
        variant: variant || "Standard",
        transmission: document.getElementById("vehicleTransmissionSelect")
          .value,
      ...
      return {
        ...
        storage:
          parseFloat(document.getElementById("elecStorageInput").value) || 256,
      };
    }
    return null;
  }
    ...
    if (cat === "gpu") {
      ...
      if (matched) {
        ...
        if (
          data.brand &&
          brandSelect &&
          Array.isArray(matched.brands) &&
          matched.brands
            .map((b) => b.toUpperCase())
            .includes(data.brand.toUpperCase())
        ) {
        ...
        showStatus(
          `Auto-detected ${matched.model} (${vramVal}GB, ${brandVal})`,
        );
      }
    } else if (cat === "mobile") {
      if (data.brand)
        document.getElementById("mobileBrandInput").value = data.brand;
      if (data.model)
        document.getElementById("mobileModelInput").value = data.model;
      if (data.storage)
        document.getElementById("mobileStorageInput").value = data.storage;
      if (data.ram) document.getElementById("mobileRamInput").value = data.ram;
      if (data.warranty_days !== undefined)
        document.getElementById("mobileWarrantyInput").value =
          data.warranty_days;
      if (data.phone_type) {
        ...
        document.getElementById("mobileTypeSelect").value =
          text.includes("iphone") || text.includes("apple")
            ? "iphone"
            : "android";
      }
    } else if (cat === "vehicle") {
      ...
      const matchedBrand = Object.keys(VEHICLE_METADATA).find(
        (b) => b.toUpperCase() === make.toUpperCase(),
      );
      if (matchedBrand) {
        populateVehicleBrands(matchedBrand);
        const sortedModels = [...(VEHICLE_METADATA[matchedBrand] || [])].sort(
        ...
        const matchedModel = sortedModels.find((m) => {
          ...
          return (
            mUp === modUp ||
            modUp.includes(mUp) ||
            (data.title && data.title.toUpperCase().includes(mUp))
          );
        });
        if (matchedModel) {
      ...
      if (data.year || data.model_year)
        document.getElementById("vehicleYearInput").value =
          data.year || data.model_year;
      if (data.variant)
        document.getElementById("vehicleVariantInput").value = data.variant;
      if (data.mileage)
        document.getElementById("vehicleMileageInput").value = data.mileage;
      if (data.engine_cc || data.engineCC)
        document.getElementById("vehicleEngineCCInput").value =
          data.engine_cc || data.engineCC;
      if (data.description)
        document.getElementById("vehicleDescriptionInput").value =
          data.description;

      const transmission = data.gear || data.transmission || "";
      if (transmission.toLowerCase().includes("auto")) {
        document.getElementById("vehicleTransmissionSelect").value =
          "Automatic";
      ...
      if (data.brand)
        document.getElementById("elecBrandInput").value = data.brand;
      if (data.model)
        document.getElementById("elecModelInput").value = data.model;
    }
  }
  ...
  function displayResults(data, listedPrice, category) {
    ...
    if (detectedCategoryTag)
      detectedCategoryTag.innerText = category.toUpperCase();
    if (detectedModelTitle) {
      const m = data.metadata
        ? data.metadata.model_name || data.metadata.model
    ...
    if (
      category === "gpu" ||
      (data.fair_market_range && data.fair_market_range.lower_price_lkr)
    ) {
      lowerPrice =
        data.lower_price ||
        (data.fair_market_range ? data.fair_market_range.lower_price_lkr : 0);
      upperPrice =
        data.upper_price ||
        (data.fair_market_range ? data.fair_market_range.upper_price_lkr : 0);
      pointPrice = data.predicted_price || 0;
      modelUsed = (data.best_model_used || "Random Forest")
        .replace("_", " ")
        .toUpperCase();
    } else if (category === "mobile") {
      pointPrice = data.predicted_price || 0;
      lowerPrice = data.fair_market_range
        ? data.fair_market_range.lower_price_lkr
        : Math.round(pointPrice * 0.9);
      upperPrice = data.fair_market_range
        ? data.fair_market_range.upper_price_lkr
        : Math.round(pointPrice * 1.1);
      ...
      lowerPrice = data.fair_market_range
        ? data.fair_market_range.lower_price_lkr
        : Math.round(pointPrice * 0.92);
      upperPrice = data.fair_market_range
        ? data.fair_market_range.upper_price_lkr
        : Math.round(pointPrice * 1.08);
    } else if (category === "electronics") {
      pointPrice =
    ...
    if (fairness && fairness.status === "OK") {
      if (verdictBanner)
        verdictBanner.className = "verdict-banner " + fairness.badgeClass;
      fairnessBadge.innerText = fairness.badgeText;
      priceDiffEl.innerText =
        fairness.score !== null ? `Score: ${fairness.score}/100` : "";
      ...
      const scoreBreakdownSection = document.getElementById(
        "scoreBreakdownSection",
      );
      const breakdownFormulaText = document.getElementById(
        "breakdownFormulaText",
      );
      const breakdownFactorsList = document.getElementById(
        "breakdownFactorsList",
      );

      if (scoreBreakdownSection && fairness.breakdown) {
        scoreBreakdownSection.classList.remove("hidden");
        if (breakdownFormulaText) {
          breakdownFormulaText.innerText =
            fairness.breakdown.formulaExplanation || "--";
        }
        if (breakdownFactorsList && Array.isArray(fairness.breakdown.factors)) {
          breakdownFactorsList.innerHTML = fairness.breakdown.factors
            .map((f) => {
