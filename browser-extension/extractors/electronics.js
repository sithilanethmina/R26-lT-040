...
window.FairPriceLK_Extractors.electronics = (function () {
    ...
    function parse(pageContext) {
        ...
        let model = key_values.model || title.replace(new RegExp(`\\b${brand}\\b`, 'gi'), '').replace(/for sale.*$/i, '').trim() || "";
        ...
        if (subCat === "monitor") {
            ...
            if (!ram) missingFields.push("RAM (GB)");
            if (!storage) missingFields.push("Storage (GB)");
            parsedData.ram = ram || 8;
            parsedData.ram_gb = ram || 8;
            parsedData.storage = storage || 256;
            parsedData.storage_gb = storage || 256;
        }
